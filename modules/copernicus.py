...
function evaluatePixel(s) {
  ...
  return { cloud_mask: [is_cloud * s.dataMask], dataMask: [s.dataMask] };
    ...
    def collect_all_indices(
        ...
        print(f"   Fetching {len(self.INDICES)} Sentinel-2 indices (parallel)...")
        ...
        if cloud:
            ...
            print(f"   ☁  Event period clouds: mean {cloud_mean:.1f}%  |  max {cloud_max:.1f}%"
            ...
            if cloud_mean > MAX_CLOUD_COVER:
                print(f"   ⚠  Cloud cover above threshold — spectral indices may"
                      f" may have low quality or N/A during this period.")
