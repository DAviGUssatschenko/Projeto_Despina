...
Satellite source: STAC Element84 / Sentinel-2 (pystac-client + rioxarray).
"""
...
# Reads the .env line by line; supports single/double quotes, BOM, CRLF, comments.
# Always writes to os.environ (override=True) so the .env always wins.
# ─────────────────────────────────────────────────────────────────────────────
def _load_env_manual():
    ...
    candidates = []
    # cwd and up to 3 parent directories
    p = Path.cwd()
    for _ in range(4):
...
def _get_poseidon_connector(db_url: str):
    """Returns a connected PoseidonConnector instance (cached per session)."""
    try:
...
def fetch_poseidon_climate(db_url: str, lat: float, lon: float,
    ...
    try:
        ...
        thresholds = _THRESH.get("seca", {})  # used internally by vote_3of4
        ...
        # IDW interpolation → daily DataFrame
        ...
        # normalise to the schema expected by the charts
...
def _idw_to_clim_df(idw_df) -> pd.DataFrame:
    ...
    Ensures columns: date, prcp, tmax, tmin, tavg, rh_avg, wspd_avg, wspd_max.
    """
    ...
    # if tavg is missing, estimate it
...
def compute_anomaly_poseidon(clim_event: pd.DataFrame, hist_baseline: dict) -> dict:
    ...
    Expected hist_baseline keys: {prcp_mean_mm, prcp_std_mm, tavg_mean_c, ...}
    Returns a dict with z-scores per variable.
    """
    ...
    for col, mu_key, sig_key, label in mappings:
        ...
        if sig and sig > 0:
            z   = (event_val - mu) / sig
            cat = ("much above normal" if z>2 else "above normal" if z>1
                   else "within normal" if z>-1 else "below normal" if z>-2
                   else "much below normal")
        else:
            pct_diff = ((event_val - mu) / (abs(mu) + 1e-9)) * 100
            z        = pct_diff / 30.0  # normaliza grosseiro
            cat      = ("above normal" if pct_diff>20 else "within normal"
                        if abs(pct_diff)<=20 else "below normal")
...
def compute_verdict(ts, clim, start, end, complaint, bioma, pos_vote=None):
    ...
    pos_w = 1.5  # Poseidon carries higher weight (real measured data)
    ws_total = tw = 0.0
...
def chart_satellite(ts: pd.DataFrame, start, end):
    ...
    fig.update_layout(**_PLOT_BASE,
        ...
        margin=dict(l=10, r=10, t=10, b=10), height=480, yaxis_title="Index value")
    if use_std:
...
def chart_clima(clim: pd.DataFrame, start, end, complaint):
    ...
    fig.add_trace(go.Bar(x=sub["date"], y=sub["prcp"].fillna(0),
        name="Daily rain", marker_color=bar_colors, opacity=0.85), row=1, col=1)
    sub["prcp_cum"] = sub["prcp"].fillna(0).cumsum()
    fig.add_trace(go.Scatter(x=sub["date"], y=sub["prcp_cum"], name="Cumulative",
...
def chart_precip_cum(clim: pd.DataFrame, start, end, complaint):
    ...
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=sub["date"], y=sub["cumsum"], name="Cumulative (mm)",
        line=dict(color=ac, width=3), fill="tozeroy", fillcolor=fill_color))
    max_p = sub["prcp"].max() or 1
    fig.add_trace(go.Bar(x=sub["date"], y=sub["prcp"].fillna(0), name="Daily (mm)",
...
def chart_water_balance(wb: pd.DataFrame, start, end, complaint):
    ...
    fig.add_trace(go.Scatter(x=sub["date"], y=sub["storage_pct"],
        name="Storage %", line=dict(color="#58a6ff", width=2.5),
    ...
    fig.add_trace(go.Bar(x=sub["date"], y=sub["runoff"], name="Surplus (mm)",
...
def build_map(rings_plot, lat, lon, ts_df, sel_index, sel_date_str):
    ...
    for ring in rings_plot:
        folium.Polygon(locations=[(p[1], p[0]) for p in ring],
            ...
            tooltip=val_label or "Event polygon").add_to(m)
    folium.CircleMarker(location=[lat, lon], radius=7, color=fill_hex,
        fill=True, fill_color=fill_hex, tooltip="Centroid").add_to(m)
...
def main():
    ...
    # ── Poseidon connection — loaded from .env / config.py ─────────────────────
    # Force re-read of .env on every Streamlit render to ensure
    # variables are available even with process-level caching.
    _load_env_manual()  # ensure re-read on every render

    def _build_db_url() -> str:
        import urllib.parse
        # 1) full connection string from config.py
        if _DB_URL and _DB_URL.strip():
            return _DB_URL.strip()
        # 2) full connection string from environment variable
        for key in ("DB_URL", "DATABASE_URL", "POSEIDON_DB_URL"):
            val = os.getenv(key, "").strip()
            if val:
                return val
        # 3) individual POSEIDON_DB_* variables
    ...
    if db_url:
        ...
        # list which individual variables are missing to aid diagnosis
    ...
    # ── Debug sidebar — shows what was read from .env ────────────────────────
    ...
    geojson_file = st.sidebar.file_uploader(
        ...
        type=["geojson", "json"],
        help=(
            "FeatureCollection with properties: id, evento, inicio, fim, solo, cultura, bioma.\n"
            "Geometry: Polygon or MultiPolygon in WGS-84 coordinates."
        ),
    )
    ...
    if not geojson_file:
        ...
        _welcome = (
            ...
            "Upload a <b style=\'color:#58a6ff\'>.geojson</b> (FeatureCollection) file in the sidebar.<br>"
            "Climate source: <b style=\'color:#bc8cff\'>Poseidon IDW</b> · "
            "Satellite: <b style=\'color:#3fb950\'>Sentinel-2 STAC</b>"
            "</div>"
            + _db_warn_html
            ...
            "<b style=\'color:#bc8cff\'>Required GeoJSON properties:</b><br>"
            "<code style=\'color:#58a6ff\'>id</code> · "
            "<code style=\'color:#58a6ff\'>evento</code> (drought/rain/frost/hail) · "
    ...
    try:
        ...
        st.error(f"Error reading GeoJSON: {e}"); return

    if gj.get("type") != "FeatureCollection":
        st.error(
            "❌ Invalid format: the file must be a **FeatureCollection**.\n\n"
            "Make sure the GeoJSON has `\"type\": \"FeatureCollection\"` "
            "with a `\"features\"` array containing features with properties and geometry."
        )
        return

    all_feat = gj.get("features", [])
    if not all_feat:
        st.error("GeoJSON has no features."); return

    # case selection (when multiple features exist)
    if len(all_feat) > 1:
        labels = [
        ...
        feat_idx = st.sidebar.selectbox("Select case", range(len(all_feat)),
    ...
    if missing:
        st.error(f"Missing or empty GeoJSON properties: **{', '.join(missing)}**"); return
    ...
    if start_dt >= end_dt:
        st.error("`inicio` must be earlier than `fim`."); return

    # geometry
    coords_flat, rings_plot, area_km2, lat, lon = parse_geometry(geom)
    if not coords_flat:
        st.error("Geometry has no valid coordinates."); return
    ...
    # ── Data fetching ─────────────────────────────────────────────────────────
    if not db_url:
        st.error("Poseidon not configured. Add DB_URL to the .env file at the project root and restart Streamlit.")
        return
    ...
    if clim_err or clim.empty:
        st.error(f"Could not retrieve climate data.\n\n{clim_err or 'Empty DataFrame.'}")
        return

    ts = pd.DataFrame()
    if HAS_STAC and HAS_GEO:
        with st.spinner("🛰️ Fetching Sentinel-2 images via STAC..."):
    ...
    # ── Header ────────────────────────────────────────────────────────────────
    evt_label = {
        "seca": "Drought / Water Deficit", "chuva": "Excess Rainfall",
        "geada": "Frost", "granizo": "Hail",
    ...
    src_tag   = ("<span style='background:#1a1f2e;color:#bc8cff;border:1px solid #bc8cff;"
                 ...
                 "font-family:Space Mono,monospace'>Poseidon IDW</span>")
    ...
    st.markdown(
        "<div class='info-grid'>"
        + info_cell("Start",  start_dt.strftime("%d/%m/%Y"))
        + info_cell("End",     end_dt.strftime("%d/%m/%Y"))
        + info_cell("Duration", f"{(end_dt - start_dt).days + 1} days")
        + info_cell("Biome",   bio_lbl if bio_lbl != "default" else "—", "font-size:13px")
        + info_cell("Area",    f"{area_ha:.1f} ha" if area_ha else "—")
        + info_cell("Images", str(n_sat))
        + (info_cell("Crop", crop_type) if crop_type else "")
        + (info_cell("Soil",    soil_name[:22], "font-size:12px") if soil_name != "default" else "")
    ...
    with tab_diag:
        ...
        # Poseidon Summary
        if pos_summ:
            st.markdown("---"); st.markdown("#### Meteorological Summary · Poseidon IDW")
            m1, m2, m3, m4, m5 = st.columns(5)
            m1.metric("Total precipitation",  f"{pos_summ.get('prcp_total_mm',0):.1f} mm")
            m2.metric("Mean temperature",   f"{pos_summ.get('tavg_mean_c',0):.1f} °C")
            m3.metric("Absolute Tmax",       f"{pos_summ.get('tmax_abs_c',0):.1f} °C")
            m4.metric("Mean relative humidity",  f"{pos_summ.get('rh_avg_mean_pct',0):.0f}%")
            m5.metric("Max wind speed",          f"{pos_summ.get('wspd_max_kmh',0):.0f} km/h")
        ...
        # anomalia histórica
        if anomaly:
            st.markdown("---"); st.markdown("#### Anomaly vs Historical · Poseidon")
        ...
        with st.expander("🔍 Debug — values vs thresholds"):
            ...
            if complaint == "chuva":
                rows_d = [_cmp("Precipitation (mm)", rp, thr["prcp_alta"], True),
    ...
    with tab_sat:
        if ts.empty:
            st.warning("No cloud-free Sentinel-2 images found for the period.")
            if not HAS_STAC:
                st.info("To enable, install: `pip install pystac-client rioxarray`")
        else:
            st.markdown("#### Spectral Index Time Series")
            st.caption(f"*{n_sat} date(s) · Sentinel-2 via STAC Element84/AWS*")
    ...
    with tab_clima:
        ...
        st.caption(f"Source: **Poseidon IDW** — lat {lat:.4f}°, lon {lon:.4f}° · "
        ...
        st.markdown(
            ...
            + _period_card("← 5 days before",  clim[(clim["date"]>=w_s)&(clim["date"]<start_dt)])
            + _period_card(f"Event ({(end_dt-start_dt).days+1} days)", clim_event, hi=True)
            + _period_card("5 days after →",  clim[(clim["date"]>end_dt)&(clim["date"]<=w_e)])
            + "</div>", unsafe_allow_html=True
        )
        ...
        with st.expander("📊 Raw data (±5 days):"):
    ...
    with tab_bal:
        ...
        if clim.empty:
            st.warning("Climate data not available.")
        else:
            try:
                ...
                st.caption(
                    ...
                    f"Total deficit in event: **{deficit:.1f} mm** · "
                    f"Surplus: **{excedent:.1f} mm**"
                )

                # mini-tabela de resumo
                col_b1, col_b2, col_b3 = st.columns(3)
                col_b1.metric("Total deficit",   f"{deficit:.1f} mm",
                              delta=f"{'critical' if deficit>50 else 'moderate' if deficit>20 else 'low'}",
                              delta_color="inverse")
                col_b2.metric("Total surplus", f"{excedent:.1f} mm")
                avg_stor = wb_evt["storage_pct"].mean() if not wb_evt.empty else 0
                col_b3.metric("Mean storage", f"{avg_stor:.0f}%",
                              delta=f"{'critical' if avg_stor<25 else 'low' if avg_stor<50 else 'ok'}",
                              delta_color="inverse")
            except Exception as e:
    ...
    with tab_map:
        ...
        if not HAS_FOLIUM:
            ...
            with col_right:
                st.markdown("<div class='side-title'>🛰️ Spectral Index</div>",
                            unsafe_allow_html=True)
                if not has_sat:
                    st.info("No satellite dates available.")
                else:
                ...
                st.markdown("<hr style='border-color:#30363d;margin:16px 0 14px'>",
                            unsafe_allow_html=True)
                st.markdown("<div class='side-title'>📅 Selected Date</div>",
                            unsafe_allow_html=True)
                if has_sat:
            ...
            with col_map:
                ...
                if available_dates and len(available_dates) > 1:
                    st.markdown("<div style='margin-top:12px;font-size:12px;color:#8b949e'>"
                                "🕒 <b>Timeline</b></div>", unsafe_allow_html=True)
