...
STATUS_LABEL = {"sim": "CONFIRMADO", "nao": "NAO CONFIRMADO",
...
def panel_pipeline_diag(analysis: dict, cop_data: dict, pos_summ: dict, pos_vote: dict):
    ...
    v_key   = {"CONFIRMADO": "sim", "NAO CONFIRMADO": "nao",
               "NÃO CONFIRMADO": "nao"}.get(verdict, "parcial")
    ...
    <div class='verdict-card verdict-{v_key}'>
      ...
      <div style='display:flex;gap:24px;margin-top:10px;flex-wrap:wrap'>
        ...
        <span style='font-size:13px;color:#8b949e'>Score IDW: <b style='color:{color}'>{idw:.0f}/100</b></span>
      </div>
    ...
    if soil_chk.get("available"):
        ...
        <div style='background:#161b22;border:1px solid #30363d;border-radius:8px;
          ...
          AWC={soil_chk.get("AWC","N/D")} mm/m · retencao: {soil_chk.get("retencao","N/D")}
          — <span style='color:{ac2}'><b>{act}</b> dano em {amp:.2f}x</span>
        </div>""", unsafe_allow_html=True)
...
def panel_standalone_diag(v_total, clim_event, hist_df):
    ...
    <div class='verdict-card {cls}'>
      <div style='font-size:13px;color:#8b949e;font-family:Space Mono,monospace;
           text-transform:uppercase;letter-spacing:.1em;margin-bottom:6px'>
        Diagnostico Standalone · Open-Meteo + Sentinel-2</div>
      <div style='font-size:32px;font-weight:700;font-family:Space Mono,monospace;color:{color}'>
    ...
    if anomaly:
        ...
        st.caption("Z-score vs baseline 10 anos · Open-Meteo")
        color_cat = {
...
def main():
    ...
    with tab_clima:
        if clim.empty:
            ...
            st.caption(f"Fonte: **{src}** — lat {lat:.4f}, lon {lon:.4f}")
            ac   = accent(complaint)
    ...
    with tab_bal:
        if clim.empty:
            ...
            try:
                ...
                st.caption(
                    f"Solo: **{soil_name}** · AWC = {awc} mm/m · Retencao: {ret} · "
                    f"Deficit total no evento: **{wb_evt['deficit'].sum():.1f} mm** · "
